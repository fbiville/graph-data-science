...
func main() {
	s, err := runQuery("bolt://demo.neo4jlabs.com:7687", "graph-data-science", "graph-data-science")
	if err != nil {
		panic(err)
	}
	fmt.Println(s)
...
func runQuery(uri, username, password string) ([]string, error) {
	...
	defer driver.Close()
	sessionConfig := neo4j.SessionConfig{AccessMode: neo4j.AccessModeRead, DatabaseName: "graph-data-science"}
